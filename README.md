...
Проект носит название iCalc, и представляет из себя распределённый вычислитель арифметических выражений.
...
```
 - В выбранной папке появится папка ```iCalc``` c проектом.

## Работа с API

### Конфигурация
#### Переменные среды
Сначала необходимо открыть файл ```config_e.env``` и установить параметры:
...
```
 - Далее надо запустить файл ```cmd_main.go```:
```
go run cmd_main.go
```
...
****Также Важно:**** Названия имеют нижнее подчеркивание, так как у меня кодер не хочет загружать папки 
### КОНЕЦ README.md
